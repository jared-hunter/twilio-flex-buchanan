...
variable "workflow_sid_inbound_call" {
  type        = string
...
variable "function_get_customer_by_phone_sid" {
  type        = string
  description = "get customer by phone function sid"
  validation {
    condition     = length(var.function_get_customer_by_phone_sid) > 2 && substr(var.function_get_customer_by_phone_sid, 0, 2) == "ZH"
    error_message = "function_get_customer_by_phone_sid expected to start with \"ZH\"."
  }
}
